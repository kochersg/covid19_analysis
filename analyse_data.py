...
def plot_single_country(
    country: str, start_date: dt = None, end_date: dt = None, save_file_name: str = None
):
    single_country = CDataTimeSeries(country=country)
    single_country_view = CDataTimeSeriesView(cv_data=single_country)
    fig = single_country_view.plot_time_series(
        show_plot=False, from_date=start_date, to_date=end_date
    )
    _save_figure(fig, save_file_name)
    plt.show()


def plot_country_collection(
    countries: list,
    start_date: dt = None,
    end_date: dt = None,
    ...
    fig = dc_view.plot_collection_subplots(
        from_date=start_date, to_date=end_date, show_plot=False
    )
    _save_figure(fig, save_file_name)
    plt.show()


def plot_doubling_time_single_country(
    country: str, start_date: dt = None, end_date: dt = None, save_file_name: str = None
):
    sc = CDataTimeSeries(country)
    c_view = CDataTimeSeriesView(sc)
    fig = c_view.plot_doubling_time_over_days(
        from_date=start_date, to_date=end_date, average_interval_days=1, show_plot=False
    )
    _save_figure(fig, save_file_name)
    plt.show()
...
def plot_simulated_data(start_date: dt = None, end_date: dt = None):
    ...
    single_country = CDataTimeSeries(
        country="Germany Sim",
        sim_data=True,
        doubling_time_dict={
            "2020-02-01": 8,
        ...
        extrapolate_to_date=dt(2020, 8, 30),
        mortality=0.045,
    )
    ...
    fig = dc_view.plot_country_comparison(
        "Germany",
        "Germany Sim",
        show_plot=False,
...
if __name__ == "__main__":
    ...
    plot_single_country(
        ...
        end_date=to_date,
        save_file_name="./example_images/SingleData.png",
    )

    # plot data of several countries into a collection
    plot_country_collection(
        ["Germany", "Cyprus", "Italy", "Spain", "United Kingdom", "Denmark"],
        start_date=from_date,
        end_date=to_date,
        save_file_name="./example_images/Collect_Subplots.png",
    )

    # plot the progress of the doubling time
    plot_doubling_time_single_country(
        "Italy",
        start_date=from_date,
        end_date=to_date,
        save_file_name="./example_images/Doubling_times.png",
    )

    # plot doubling time for selected countries
    plot_doubling_time_collection(
        [
            "Germany",
            "Cyprus",
            "United Kingdom",
